...
public class JustificativoVista extends AncestroVista {
	...
	@SuppressWarnings("rawtypes")
	private JComboBox cbFaltas;

	@SuppressWarnings({ "rawtypes", "unchecked" })
	public JustificativoVista() throws ClassNotFoundException {
		...
		setTitle("PERSONAL - ASIGNACION DE JUSTIFICATIVO");
		...
		btnFuncionario.setIcon(new ImageIcon(
				JustificativoVista.class.getResource("/com/migraciones/talentoHumano/graphics/lupa.png")));
		...
		btnOficina.setIcon(new ImageIcon(
				JustificativoVista.class.getResource("/com/migraciones/talentoHumano/graphics/lupa.png")));
		...
		lblNewLabel.setIcon(new ImageIcon(
				JustificativoVista.class.getResource("/com/migraciones/talentoHumano/graphics/busqueda.png")));
		lblNewLabel.setBounds(657, 44, 157, 150);
		panelCampos.add(lblNewLabel);

		JPanel pnBusqueda = new JPanel();
		pnBusqueda.setBorder(
				new TitledBorder(null, "Busqueda de Faltas", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		pnBusqueda.setBackground(new Color(255, 222, 173));
		pnBusqueda.setBounds(343, 129, 313, 65);
		panelCampos.add(pnBusqueda);
		pnBusqueda.setLayout(null);

		btnFaltas = new JButton("");
		btnFaltas.setEnabled(false);
		btnFaltas.setToolTipText("Buscar faltas");
		btnFaltas.setSelectedIcon(new ImageIcon(
				JustificativoVista.class.getResource("/com/migraciones/talentoHumano/graphics/actualizar.png")));
		btnFaltas.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					btnFaltasActionPerformed(e);
		...
		btnFaltas.setIcon(new ImageIcon(
				JustificativoVista.class.getResource("/com/migraciones/talentoHumano/graphics/lupa.png")));
		btnFaltas.setBounds(278, 30, 25, 21);
		pnBusqueda.add(btnFaltas);

		cbFaltas = new JComboBox();
		cbFaltas.setEnabled(false);
		cbFaltas.setModel(new DefaultComboBoxModel(new String[] { "-- SELECCIONE --", "AUSENCIAS SIN JUSTIFICAR" }));
		cbFaltas.setBounds(10, 30, 262, 20);
		pnBusqueda.add(cbFaltas);
		formatearTabla();
	...
	protected void btnFaltasActionPerformed(ActionEvent e)
		...
		if (dcDesde.getDate() == null || dcHasta.getDate() == null || verificarPeriodo()
				|| cbFaltas.getSelectedIndex() == 0) {
			JOptionPane.showMessageDialog(null, GlobalUtil.MSG_VERIFICAR_PERIODO, "ATENCION",
					JOptionPane.ERROR_MESSAGE);
		} else {
			FaltaDisciplinariaModal vista = new FaltaDisciplinariaModal(txtCedula.getText(), this.fechaDesde, this.fechaHasta);
	...
	protected void agregarObjeto() {
		...
		this.setTitle("PERSONAL - ASIGNACION DE JUSTIFICACIONES   (AGREGANDO)");
		this.opcion = 1;
		activarFormulario();
		limpiarTabla();
	...
	protected void editarObjeto() throws ClassNotFoundException {
		...
		if (tbJustificaciones.getSelectedRow() != -1
			...
			this.setTitle("PERSONAL - ASIGNACION DE JUSTIFICACIONES (MODIFICANDO)");
			this.opcion = 2;
			activarFormulario();
	...
	protected void borrarObjeto() throws ClassNotFoundException {
		...
		if (tbJustificaciones.getSelectedRow() != -1
				&& tbJustificaciones.getValueAt(tbJustificaciones.getSelectedRow(), 0) != null) {
			this.setTitle("PERSONAL - ASIGNACION DE JUSTIFICACIONES (ELIMINANDO)");
			this.opcion = 3;
			activarFormulario();
			cargarDatosModificar();
			txtCedula.setEnabled(false);
	...
	protected void cancelarOperacion() {
		...
		this.setTitle("PERSONAL - ASIGNACION DE JUSTIFICACIONES");
		desactivarFormulario();
		this.panelBotones.deshabilitar();
