...
public class CategoriaModal extends AncestroModal {
	private int COLUMNA_CODIGO = 0;
	private int COLUMNA_CATEGORIA = 1;
	...
	public CategoriaModal() throws ClassNotFoundException {
	...
	private void cargarTabla() throws ClassNotFoundException {
		...
		TableColumn columna1 = tbConsultas.getColumn("CATEGORIA");
		...
		for (Categoria cat : sistema.obtenerCategorias()) {
			fila[0] = cat.getCodigo();
			fila[1] = cat.getDescripcion();
	...
	private void cargarCategoria() throws ClassNotFoundException {
		String codigo = (String) tbConsultas.getValueAt(tbConsultas.getSelectedRow(), COLUMNA_CODIGO);
		String categoria = (String) tbConsultas.getValueAt(tbConsultas.getSelectedRow(), COLUMNA_CATEGORIA);
		this.codigo = codigo;
	...
	private void filtrar() {
		...
		try {
			rf = RowFilter.regexFilter(txtBuscador.getText().toUpperCase(),COLUMNA_CODIGO ,COLUMNA_CATEGORIA);
