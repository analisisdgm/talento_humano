...
public class CargoModal extends AncestroModal {
	private int COLUMNA_CODIGO = 0;
	private int COLUMNA_CARGO = 1;
	...
	public CargoModal() throws ClassNotFoundException {
		setTitle("SELECCIONE CARGO");
	...
	private void cargarTabla() throws ClassNotFoundException {
		...
		TableColumn columna1 = tbConsultas.getColumn("CARGO");
		...
		for (Cargo car : sistema.obtenerCargos()) {
			fila[0] = Integer.toString(car.getCodigo());
			fila[1] = car.getDescripcion();
	...
	private void cargarPersonal() throws ClassNotFoundException {
		...
		String cargo = (String) tbConsultas.getValueAt(tbConsultas.getSelectedRow(), COLUMNA_CARGO);
		this.codigo = codigo;
		this.cargo = cargo;
	}
	...
	private void filtrar() {
		...
		int indiceColumnaTabla = COLUMNA_CARGO;
		try {
